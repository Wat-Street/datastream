...
**Stale-proposal conflicts.** A 409 carries a `conflict_type` in its body (`ApiError.body`, exposed via `isProposalConflictBody`) alongside `detail`. A `dataset_exists` conflict is a hard block — just the inline error box, same as any other 400/409. A `stale_branch` conflict (the proposal branch exists from an earlier, never-registered proposal — e.g. a previous test PR was closed without deleting its branch) instead opens a confirm dialog naming the still-open PR if `open_pr_url` is present. Confirming resubmits the same form values with `override: true`, which the server resolves by closing the stale PR and deleting the branch before opening a fresh one; a conflict that survives that (e.g. a race) falls through to the ordinary inline error box.
