...
def test_load_all_configs_empty_dir(tmp_path: Path) -> None:
    ...
    assert registry._CONFIG_REGISTRY == {}
...
def test_load_all_configs_single(tmp_path: Path, write_config: Callable) -> None:
    ...
    registry.load_all_configs(tmp_path)
    assert (("ds", V010)) in registry._CONFIG_REGISTRY
    cfg = registry._CONFIG_REGISTRY[("ds", V010)]
...
def test_load_all_configs_multiple(tmp_path: Path, write_config: Callable) -> None:
    ...
    registry.load_all_configs(tmp_path)
    assert len(registry._CONFIG_REGISTRY) == 3
    assert ("a", V010) in registry._CONFIG_REGISTRY
    assert ("b", V010) in registry._CONFIG_REGISTRY
    assert ("b", V020) in registry._CONFIG_REGISTRY
...
def test_load_all_configs_with_valid_dependency(
    ...
    registry.load_all_configs(tmp_path)
    assert len(registry._CONFIG_REGISTRY) == 2
...
def test_load_all_configs_skips_dirs_without_config_toml(
    ...
    registry.load_all_configs(tmp_path)
    assert len(registry._CONFIG_REGISTRY) == 1
...
""",
    ...
    assert len(registry._CONFIG_REGISTRY) == 4
