...
def _query_rows(db_conn, dataset_name, version):
...
def test_build_root_single_day(client, db_conn):
    ...
    rows = _query_rows(db_conn, "mock-ohlc", "0.1.0")
    assert len(rows) == 1
...
def test_build_single_dep_chain(client, db_conn):
    ...
    ohlc_rows = _query_rows(db_conn, "mock-ohlc", "0.1.0")
...
def test_build_multi_row(client, db_conn):
    ...
    rows = _query_rows(db_conn, "mock-multi-ohlc", "0.1.0")
    assert len(rows) == 3
...
def test_build_multi_row_dep_chain(client, db_conn):
    ...
    ohlc_rows = _query_rows(db_conn, "mock-multi-ohlc", "0.1.0")
    assert len(ohlc_rows) == 3

    close_rows = _query_rows(db_conn, "mock-multi-close", "0.1.0")
...
def test_build_lookback(client, db_conn):
    ...
    avg_rows = _query_rows(db_conn, "mock-moving-avg", "0.1.0")
...
def test_build_lookback_range(client, db_conn):
    ...
    for avg_row in avg_rows:
        ...
        if not prices:
            pytest.fail(f"no close prices found in lookback window for {avg_ts}")
